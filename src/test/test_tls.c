...
#include <string.h>
#include <assert.h>
#include "../netguard/include/tls.h"
...
const unsigned char good_data_2[] = {
        ...
        0x00, 0x07, // Length
        // "ddg.com"
        0x64, 0x64, 0x67, 0x2e, 0x63, 0x6f, 0x6d,
        // Extension
...
const unsigned char bad_data_1[] = {
        0x16, 0x03, 0x01, 0x00, 0x58, 0x01, 0x00, 0x00, // wrong length, 0x58 -> 0x68
...
int main() {
    ...
    *sn = 0;
    error = get_server_name(pkt, sizeof(good_data_1), pkt, sn);
    assert(strcmp("localhost", sn) == 0);
    assert(error == strlen(sn));

    pkt = (uint8_t *)good_data_2;
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    error = get_server_name(pkt, sizeof(good_data_2), pkt, sn);
    assert(strcmp("ddg.com", sn) == 0);
    assert(error == strlen(sn));

    pkt = (uint8_t *)good_data_3;
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    error = get_server_name(pkt, sizeof(good_data_3), pkt, sn);
    assert(strcmp("localhost", sn) == 0);
    assert(error == strlen(sn));

    pkt = (uint8_t *)good_data_4;
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    error = get_server_name(pkt, sizeof(good_data_4), pkt, sn);
    assert(strcmp("localhost", sn) == 0);
    assert(error == strlen(sn));

    pkt = (uint8_t *)good_data_5;
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    error = get_server_name(pkt, sizeof(good_data_5), pkt, sn);
    assert(strcmp("localhost", sn) == 0);
    assert(error == strlen(sn));

    pkt = (uint8_t *)ssl30_request;
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    ...
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    error = get_server_name(pkt, sizeof(ssl20_client_hello), pkt, sn);
    assert(strcmp("localhost", sn) != 0);
    assert(strlen(sn) == 0);
    assert(error == -2);

    pkt = (uint8_t *)bad_data_1;
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    ...
    *sn = 0;
    ...
    memset(sn, 0, FQDN_LENGTH);
    *sn = 0;
    error = get_server_name(pkt, sizeof(bad_data_3), pkt, sn);
    assert(strcmp("localhost", sn) != 0);
    assert(strlen(sn) == 0);
    assert(error == -1);
